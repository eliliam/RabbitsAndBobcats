...
const gens = 40; // Amount of generations to run through

const maxRabbits = 300; // Carrying capacity of rabbits
...
function spawnNewRabbits(curMap){
    ...
    for (let r = 0; r < startingPop && startingPop <= 150; r++) {
        let x = Math.ceil(Math.random() * mapSize - rabbitSize);
        let y = Math.ceil(Math.random() * mapSize - rabbitSize);
        if (!curMap[x][y]) {
            curMap[x][y] = 1;
...
const rfinal = eatenOverTime.map((val) => (val / maxEaten)*10);
const bfinal = bobcatsOverTime.map((val) => ((val-1) / (maxBobcats-1))*10);
...
console.log(("Rabbits eaten over "+gens+" generations").blue);
console.log(asciichart.plot(rfinal));
console.log(("Bobcats alive over time "+gens+" generations").blue);
